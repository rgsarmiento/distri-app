<x-app-layout>
    <x-slot name="header">Editar Orden #{{ $order->id }}</x-slot>
        ...
        <form action="{{ route('orders.update', $order->id) }}" method="POST" id="order_form">
            ...
            <div class="grid-cols-mobile-1" style="display:grid;grid-template-columns:1fr;gap:24px;margin-bottom:24px;">
                ...
                <div>
                    ...
                    <input type="text" id="customer_search" class="form-input" value="{{ $order->customer->full_name }} - {{ $order->customer->identification }}" placeholder="Buscar por nombre o identificación..." autocomplete="off" oninput="filterCustomers()">
            ...
            <div id="order_totals" style="margin-top:32px;padding-top:24px;border-top:2px solid #F3F4F8;display:block;">
                <div style="display:flex; flex-direction:column; gap:24px;">
                    <div>
                        <label class="form-label">Observaciones del Pedido</label>
                        <textarea name="observations" id="observations" class="form-input" rows="3" placeholder="Añade notas adicionales aquí..." style="width:100% !important; max-width:100%;">{{ $order->observations }}</textarea>
                    </div>
                    ...
                    <div style="max-width:300px;margin-left:auto;display:flex;flex-direction:column;gap:10px; width:100%;">
                        <div style="display:flex;justify-content:space-between;color:#6B7280;font-size:14px;">
                            <span>Subtotal</span>
                            <span id="total_base_price">$<span>{{ number_format($order->subtotal, 2) }}</span></span>
                        </div>
                        <div style="display:flex;justify-content:space-between;color:#6B7280;font-size:14px;">
                            <span>Impuestos</span>
                            <span id="total_tax">$<span>{{ number_format($order->total_tax, 2) }}</span></span>
                        </div>
                        <div style="display:flex;justify-content:space-between;font-size:18px;font-weight:800;color:#1E1B2E;border-top:1.5px solid #E5E7EB;padding-top:10px;">
                            ...
                            <span id="total_price" style="color:#6C3DE0;">$<span>{{ number_format($order->total, 2) }}</span></span>
                        </div>
                        
                        <div style="display:flex; gap:12px; margin-top:12px;">
                            <a href="{{ route('orders.index') }}" class="btn-secondary" style="flex:1; justify-content:center; padding:14px;">Cancelar</a>
                            <button type="submit" class="btn-primary" style="flex:2; justify-content:center; padding:14px; font-size:16px;">
                                Guardar Cambios
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        window.addEventListener('load', () => {
            const existingProducts = @json($order->products);
            existingProducts.forEach(p => {
                renderProductItem({
                    priceKey: `${p.id}_${p.pivot.subtotal / p.pivot.quantity}`,
                    productId: p.id,
                    name: p.name,
                    code: p.code,
                    basePrice: p.pivot.subtotal / p.pivot.quantity,
                    taxRate: p.tax_rate,
        ...
        function updateTotals() {
            ...
            document.querySelector('#total_price span').textContent = (totalBasePrice + totalTax).toLocaleString('es-CO', {minimumFractionDigits: 2});
        }
    </script>
</x-app-layout>
